...
void Topic2D::applyPointCloud2D(uint16_t* _distance_buffer_2d)
{
    ...
    for (uint8_t i = 0; i < DATA_LENGTH_2D; i++)
    {
        ...
        if (_distance_buffer_2d[buffer_index] < D2_Const::DISTANCE_MAX_VALUE_2D)
        {
...
void Topic2D::assignLaserScan(const std::string& _frame_id)
{
    ...
    message_laserscan->angle_max       =  D2_Const::HORIZONTAL_ANGLE / 2.0f * ROS_Const::DEGREE2RADIAN;
    message_laserscan->angle_increment =  D2_Const::ANGLE_INCREMENT_2D * ROS_Const::DEGREE2RADIAN;
    message_laserscan->scan_time       = 0;
    message_laserscan->range_min       = D2_Const::DISTANCE_MIN_VALUE_2D * ROS_Const::MM2M;
    message_laserscan->range_max       = D2_Const::DISTANCE_MAX_VALUE_2D * ROS_Const::MM2M;
