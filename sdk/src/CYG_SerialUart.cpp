...
uint16_t CYG_SerialUart::getPacketLength(uint8_t* _received_buffer)
{
    ...
    number_of_packet = serial_port->read_some(boost::asio::buffer(_received_buffer, D2_Const::SCAN_MAX_SIZE), error_code);
...
void CYG_SerialUart::requestRunMode(const uint8_t _run_mode, std::string &_notice)
{
    ...
    switch (_run_mode)
    {
        case ROS_Const::MODE_2D:
            payload_buffer.push_back(D2_Const::SEND_DEPTH_2D);
            _notice = "RUN 2D MODE";
            break;
        case ROS_Const::MODE_3D:
            payload_buffer.push_back(D2_Const::SEND_DEPTH_3D);
            _notice = "RUN 3D MODE";
            break;
        case ROS_Const::MODE_DUAL:
            payload_buffer.push_back(D2_Const::SEND_DEPTH_DUAL);
            _notice = "RUN DUAL MODE";
            break;
    }
    payload_buffer.push_back(D2_Const::COMMAND_DATA);
...
void CYG_SerialUart::requestSwitch3DType(const uint8_t _3d_mode, std::string &_notice)
{
    ...
    if (_3d_mode == ROS_Const::MODE_DISTANCE)
    {
        payload_buffer.push_back(ROS_Const::MODE_DISTANCE);
        _notice = "SWITCH TO DISTANCE TYPE";
    }
    else if (_3d_mode == ROS_Const::MODE_AMPLITUDE)
    {
        payload_buffer.push_back(ROS_Const::MODE_AMPLITUDE);
        _notice = "SWITCH TO DISTANCE & AMPLITUDE TYPE";
    }

    transferPacketCommand(payload_buffer);
}

void CYG_SerialUart::requestDurationControl(const uint8_t _run_mode, const uint8_t _duration_mode, uint16_t _duration_value)
{
    payload_buffer.clear();
    ...
    payload_buffer.push_back(D2_Const::INTEGRATION_TIME);
...
{
    payload_buffer.clear();
    payload_buffer.push_back(D2_Const::FREQUENCY_CHANNEL);
...
{
    payload_buffer.clear();
    payload_buffer.push_back(D2_Const::DEVICE_INFO);
    payload_buffer.push_back(D2_Const::COMMAND_DATA);
...
{
    payload_buffer.clear();
    payload_buffer.push_back(D2_Const::SET_BAUDRATE);
...
void CYG_SerialUart::requestNewFiltering(const uint8_t _run_mode, const uint8_t _filter_mode, std::string &_notice)
{
    ...
    payload_buffer.clear();
    payload_buffer.push_back(D2_Const::SET_NEW_FILTERING);

    if (_filter_mode == ROS_Const::NONE_FILTER)
    {
        payload_buffer.push_back(_filter_mode);
        _notice = "NONE FILTER APPLIED";
    }
    else if (_filter_mode == ROS_Const::MEDIAN_FILTER)
    {
        payload_buffer.push_back(_filter_mode);
        _notice = "MEDIAN FILTER APPLIED";
    }
    else if (_filter_mode == ROS_Const::AVERAGE_FILTER)
    {
        payload_buffer.push_back(_filter_mode);
        _notice = "AVERAGE FILTER APPLIED";
    }

    transferPacketCommand(payload_buffer);
}

void CYG_SerialUart::requestEdgeFiltering(const uint8_t _run_mode, uint16_t _edge_filter_value)
{
    payload_buffer.clear();
    ...
    payload_buffer.push_back(D2_Const::SET_EDGE_FILTERING);
    payload_buffer.push_back(LSB);
    payload_buffer.push_back(MSB);
...
{
    payload_buffer.clear();
    payload_buffer.push_back(D2_Const::SEND_STOP);
    payload_buffer.push_back(D2_Const::COMMAND_DATA);
...
void CYG_SerialUart::transferPacketCommand(const std::vector<uint8_t> &payload)
{
    ...
    command_buffer.push_back(D2_Const::NORMAL_MODE);
    command_buffer.push_back(D2_Const::PRODUCT_CODE);
    command_buffer.push_back(D2_Const::DEFAULT_ID);
